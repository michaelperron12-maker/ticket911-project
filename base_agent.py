...
try:
    ...
    load_dotenv("/var/www/aiticketinfo/.env")
except ImportError:
    pass
...
KIMI_THINK = "accounts/fireworks/models/kimi-k2-thinking"      # Thinking mode audit 262K ctx

# --- RETIRE ---
COGITO = KIMI_THINK                                             # Cogito 671B plus dispo sur Fireworks, remplace par Kimi K2 Thinking
...
DATA_DIR = "/var/www/aiticketinfo/data"
LOG_DIR = "/var/www/aiticketinfo/logs"
...
class CanLIIRateLimiter:
    ...
    _daily_reset_day = 0
    _consecutive_429 = 0
    _usage_file = "/var/www/aiticketinfo/logs/canlii_usage.json"
...
class BaseAgent:
    ...
    FALLBACK_CHAIN = [DEEPSEEK_V3, DEEPSEEK_R1, MIXTRAL_FR, KIMI_K2, GLM4, MINIMAX]
    ...
    def call_ai(self, prompt, system_prompt="", model=None, temperature=0.1, max_tokens=2000):
        ...
        try:
            ...
            try:
                ...
                # model pas dans la chain — essayer KIMI_K2 comme fallback general
                if model != KIMI_K2:
                    self.log(f"Fallback {model.split('/')[-1]} → kimi-k2p5", "WARN")
                    return self.call_ai(prompt, system_prompt, model=KIMI_K2, temperature=temperature, max_tokens=max_tokens)
